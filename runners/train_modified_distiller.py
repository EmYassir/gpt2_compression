...
from distillation.modified_distiller import Distiller, require_teacher
...
def main():
    ...
    args.max_model_input_size = tokenizer.max_model_input_sizes[args.teacher_type] ## YE: change back to teacher_name
    ...
    if extension == "txt":
        extension = "text"
    train_lm_seq_dataset = load_dataset(extension, data_files=data_files, ignore_verifications=True)
    logger.info("Data loaded successfully.")

    # Preprocessing the datasets.
    # First we tokenize all the texts.
    column_names = train_lm_seq_dataset["train"].column_names
    text_column_name = "text" if "text" in column_names else column_names[0]

    def tokenize_function(examples):
        return tokenizer(examples[text_column_name])
    logger.info("Tokenizing datasets...")
    tokenized_datasets = train_lm_seq_dataset.map(
        tokenize_function,
        batched=True,
        num_proc=2,
    ...
    logger.info("Dataset grouped successfully")
    train_lm_seq_dataset = lm_datasets["train"]
    ...
    if require_teacher(args):
        ...
        teacher = None
        logger.info(f"Won't load Teacher from {args.teacher_name}.")
    # FREEZING #
