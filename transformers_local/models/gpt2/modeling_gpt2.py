...
class GPT2Attention(nn.Module):
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None):
        ...
        # YE: We have to return attention weights before softmax to avoid numerical issues
        attn_weights_soft = nn.Softmax(dim=-1)(attn_weights)
        attn_weights_drop = self.attn_dropout(attn_weights_soft)

        # Mask heads if we want to
        if head_mask is not None:
            attn_weights_drop = attn_weights_drop * head_mask

        attn_output = torch.matmul(attn_weights_drop, value)
...
class GPT2ForSpanClassification(GPT2PreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"h\.\d+\.attn\.masked_bias", r"lm_head\.weight"]
    ...
    def __init__(self, config):
        ...
        self.transformer = GPT2Model(config)
        self.score = nn.Linear(config.num_spans * config.n_embd, self.num_labels, bias=False)
    ...
    def forward(
        ...
        pooled_logits = self.score(span_reps)
        if input_ids is not None:
            batch_size, sequence_length = input_ids.shape[:2]
        else:
            batch_size, sequence_length = inputs_embeds.shape[:2]
