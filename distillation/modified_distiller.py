...
from utilities.loss_objects import RMSELoss
...
def require_teacher(args):
    return ((args.alpha_ce != 0.0) or (args.alpha_mse != 0.0) or (args.alpha_cos != 0.0))
...
class Distiller:
    def __init__(
        ...
        if self.alpha_mse > 0.0:
            #self.mse_loss_fct = nn.MSELoss(reduction="sum") YE: change to scale the weights
            self.mse_loss_fct = RMSELoss()
        if self.alpha_cos > 0.0:
        ...
        self.optimizer = AdamW(
            optimizer_grouped_parameters, lr=params.learning_rate, eps=params.adam_epsilon, betas=(0.9, 0.999) #betas=(0.9, 0.98)
        )
    ...
    def prepare_batch_clm(self, batch):
        ...
        """
        logger.info("Creating lengths ...")
        token_ids = batch
        device = token_ids.get_device()
        lengths = torch.tensor([len(t) for t in batch], dtype=torch.int32, device=device)
        logger.info("Rounding batches ...")
        token_ids, lengths = self.round_batch(x=token_ids, lengths=lengths)
        assert token_ids.size(0) == lengths.size(0)
    ...
    def step(self, input_ids: torch.tensor, attention_mask: torch.tensor, lm_labels: torch.tensor):
        ...
        if self.mlm:
            ...
            output = self.student(
                input_ids=input_ids, attention_mask=None, labels=lm_labels
            ) 
            s_logits, s_hidden_states = output.logits, output.hidden_states # (bs, seq_length, voc_size)
            s_loss = output.loss
            if require_teacher(self.params):
                with torch.no_grad():
                    output = self.teacher(
                    ...
                    t_logits,  t_hidden_states = output.logits, output.hidden_states # (bs, seq_length, voc_size)
        if require_teacher(self.params):
        ...
        # Init all losses
        loss_ce, loss_clm, loss_cos, loss_mse, loss = 0., 0., 0., 0., 0. 
